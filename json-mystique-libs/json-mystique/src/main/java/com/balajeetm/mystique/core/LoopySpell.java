...
public class LoopySpell implements Spell {
   ...
   *
   * @see
   * com.futuresight.util.mystique.Spell#cast(com.futuresight.util.mystique.
   * Mystique)
   */
  @Override
  public JsonElement cast(MystTurn mystique) {
    ...
    if (CollectionUtils.isNotEmpty(source)) {
      ...
      for (JsonElement jsonElement : jsonArray) {
        ...
        CompletableFuture<JsonElement> transformElement =
            CompletableFuture.supplyAsync(
                    () ->
                        mystique.transform(
                            Lists.newArrayList(jsonElement),
                            dependencies,
                            aces,
                            turn,
                            resultWrapper))
                .exceptionally(
                    e -> {
                      String msg =
                          String.format(
                              "Error transforming one of the elements in the array for %s - %s",
                              turn, e.getMessage());
                      logger.info(msg, e);
